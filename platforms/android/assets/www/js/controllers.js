...
.controller('facebookFriendsCtrl', function($scope, facebookService) {
    ...
    // This is called with the results from from FB.getLoginStatus().
          function statusChangeCallback(response) {
            ...
            if (response.status === 'connected') {
              ...
              document.getElementById('fblogin').innerHTML ="Welcome User";              
              gmap($scope, facebookService);
    ...
    function gmap($scope, facebookService){
       var map = null;
        ...
        facebookService.getMyLastName().then(function(response) { //alert("response"); 
            console.log(response);
         }
       );                
...
.controller('MapCtrl', function($scope, $ionicHistory, $ionicLoading, $filter , $ionicNavBarDelegate, $timeout, $state) {
    ...
    var TextOfDirectionSting = 0;  
      ...
      function initialize() {
        ...
        mycitycenter = new google.maps.LatLng(30.731212, 76.830220);
        ...
        var ShoeIsland = new google.maps.LatLng(30.728835, 76.845290);
          ...
          for(i=0; i<customMarker.length; i++){
            var distance = google.maps.geometry.spherical.computeDistanceBetween(myLatlng, customMarker[i]['position']);
            customMarker[i]['distance'] = distance; 
          }
          var first = 0; 
          var iii;
          iii = $filter('orderBy')(customMarker, 'distance'); 
          
          for(i=0; i<iii.length; i++){
            
            if(first == 0){
                var marker_img = 'img/keep-left.png';
                first = 1;
            } else {
                var marker_img = 'img/restaurant.png';
            }
            
            marker = new google.maps.Marker({
                position: iii[i]['position'],
                map: map,
                icon: marker_img,
                title: iii[i]['title']
            });
          ...
          function onSuccess(pos){
            ...
            var distance212 = google.maps.geometry.spherical.computeDistanceBetween(myLatlng, troute.start_point);
                   ...
                   var mmn = route.legs[0].steps[ddd1].distance.value;
                    ...
                    if(ddd1 == 0){
                        var checkDistace = 100;
                    } else {
                        var checkDistace = 50;
                    }
                    if(distance212 <= checkDistace){
                        if(ddl == 1){
                            ddd1 = ddd1 + 1;
                            ddl = 0;    
                        } else {
